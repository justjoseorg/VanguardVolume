...
Map G1-G6 to **F13-F18** in iCUE or Corsair Web Hub. G1 selects Windows master volume. G2-G6 receive stable assignments to active shared-mode audio applications. After selecting a target, the keyboard's volume dial adjusts that target and dial press toggles mute. A non-activating desktop flyout shows the current mapping.

The VANGUARD display is deliberately not driven by this MVP: Corsair has not published a supported live framebuffer/LCD API. The flyout uses the same mixer state and is the supported dynamic display transport.
